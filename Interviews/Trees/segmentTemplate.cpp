...
int get_range_min(int *segment_tree,int i, int s, int e, int qs, int qe) {
    // No overlap
    if(qs > e or qe < s) {
        return INT_MAX;
    }
    ...
    // Partial overlap
    int mid = (s + 2) /2;
    int leftAns = get_range_min(segment_tree, 2 * i + 1, s, mid, qs, qe);
    int rightAns = get_range_min(segment_tree, 2 * i + 2, mid + 1, e, qs, qe);
    return min(leftAns, rightAns);
}

/* update the arr[idx] with value: val*/
void update_node(int *arr, int *segment_tree,int i, int s, int e, int idx, int val) {
    if(i < s or i > e) {
        return;
    }
    if(s == e) {
        ...
        int mid = (s + e) /2;
        update_node(arr, segment_tree,2 * i + 1,  s, mid, idx, val);
        update_node(arr, segment_tree,2 * i + 2, mid + 1, e, idx, val);
